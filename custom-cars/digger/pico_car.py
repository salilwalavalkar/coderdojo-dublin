...
# this class translates the basic forward back left right used in dojo to
# what the car expects, the continually variying feeds from a thumbstick
# dojo has separate speeds for each wheel, needs to rip out the atmega's
# & reflash to facilitate that. Another day!
class pico_car:
    Arm = pico_arm()
    ...
    def Car_Run(self, speed1, speed2):
        self.Arm.direction(128)
        self.Arm.backward(128,128)
        self.Arm.forward(speed1,speed2)
        
    def Car_Stop(self):
        self.Arm.forward(128,128)
        self.Arm.backward(128,128)
        self.Arm.direction(128)
    ...
    def Car_Back(self, speed1, speed2):
        self.Arm.direction(128)
        self.Arm.forward(128,128)
        self.Arm.backward(speed1, speed2)
        
    def Car_Left(self, speed1, speed2):
        self.Arm.forward(128,128)
        self.Arm.backward(128,128)
        self.Arm.direction(128-int(speed1/2))
    ...
    def Car_toggleLed(self):
        self.Arm.toggleLed()

    def Car_armLeft(self, go):
        if( go > 0 ):
            self.Arm.baseLeft(1)
        else:
            self.Arm.baseLeft(0)

    def Car_armRight(self, go):
        if( go > 0 ):
            self.Arm.baseRight(1)
        else:
            self.Arm.baseRight(0)

    def Car_armJointUp(self, joint, speed):
        self.Arm.joint(joint, 1, speed)

    def Car_armJointDown(self, joint, speed):
        self.Arm.joint(joint, 0, speed)

    def Car_grip(self, speed):
        self.Arm.grip(1, speed)

    def Car_release(self, speed):
        self.Arm.grip(0, speed)
...
class SSD1306_I2C(SSD1306):
    ...
    def write_cmd(self, cmd):
        ...
        self.temp[1] = cmd
        # SSD1306_I2C not implemented in the arm car
